...
const ButtonFriend = ({ user_one_id, user_two_id }) => {
  ...
  const handleAddFriend = () => {
        if(window.confirm('xác nhận kết bạn')) {
            fetch("/api/friends", {
              ...
              .then((data) => {
                if (data.status === "success") setStatus('2');
  ...
  if (status === "3") {
    return (
      <div className="positon-relative">
        <button className="btn btn-primary end-50 position-absolute"  onClick={handleAddFriend}>
          Thêm bạn
        </button>
      </div>
    );
  ...
  if (status === "2") {
    return (
      <div className="positon-relative">
        <button className="btn btn-primary end-50 position-absolute" onClick={handleDeleteFriend}>
          Hủy kết bạn
        </button>
      </div>
    );
